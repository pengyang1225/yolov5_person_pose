...
from utils.loss import compute_loss,compute_loss_awl
...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        logger.info(('\n' + '%10s' * 10) % ('Epoch', 'gpu_mem', 'box', 'obj', 'cls', 'landmark','  point_conf_loss', 'total', 'targets', 'img_size'))
        if rank in [-1, 0]:
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                loss, loss_items = compute_loss_awl(pred, targets.to(device), model,awl)  # loss scaled by batch_size
                if rank != -1:
