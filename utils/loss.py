...
from utils.bce_loss import cross_entropy_sigmoid_loss

def compute_loss_awl(p, targets, model,AWL):  # predictions, targets, model
    device = targets.device
    lcls, lbox, lobj, lmark,point_cls_loss = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device),torch.zeros(1, device=device)
    tcls, tbox, indices, anchors, tlandmarks, lmks_mask,conf_point = build_targets(p, targets, model)  # targets
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if n:
            ...
            if model.nc > 1:  # cls loss (only if multiple classes)
                t = torch.full_like(ps[:, 29:], cn, device=device)  # targets
                t[range(n), tcls[i]] = cp
                lcls += BCEcls(ps[:, 29:], t)  # BCE
            ...
            plandmarks[:, 10:12] = plandmarks[:, 10:12] * anchors[i]
            plandmarks[:, 12:14] = plandmarks[:, 12:14] * anchors[i]
            plandmarks[:, 14:16] = plandmarks[:, 14:16] * anchors[i]
            plandmarks[:, 16:18] = plandmarks[:, 16:18] * anchors[i]
            plandmarks[:, 18:20] = plandmarks[:, 18:20] * anchors[i]
            plandmarks[:, 20:22] = plandmarks[:, 20:22] * anchors[i]
            plandmarks[:, 22:24] = plandmarks[:, 22:24] * anchors[i]

            lmark += landmarks_loss(plandmarks, tlandmarks[i], lmks_mask[i])

            p_point = ps[:, 29:41]

            point_cls_loss +=cross_entropy_sigmoid_loss(p_point,conf_point[i])
   ...
   # loss = lbox + lobj + lcls + lmark
    loss = lbox + lobj + lcls
    loss_sum = AWL(loss, lmark *0 ,point_cls_loss *0)
    return loss_sum * bs, torch.cat((lbox, lobj, lcls, lmark, point_cls_loss,loss_sum)).detach()
...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if n:
            ...
            plandmarks[:, 8:10] = plandmarks[:,8:10] * anchors[i]
            plandmarks[:, 10:12] = plandmarks[:, 10:12] * anchors[i]
            plandmarks[:, 12:14] = plandmarks[:, 12:14] * anchors[i]#x7
            plandmarks[:, 14:16] = plandmarks[:, 14:16] * anchors[i]
            plandmarks[:, 16:18] = plandmarks[:, 16:18] * anchors[i]
            plandmarks[:, 18:20] = plandmarks[:, 18:20] * anchors[i]
            plandmarks[:, 20:22] = plandmarks[:, 20:22] * anchors[i]
...
def build_targets(p, targets, model):
    ...
    tcls, tbox, indices, anch, landmarks, lmks_mask,conf_point = [], [], [], [], [], [],[]
    #gain = torch.ones(7, device=targets.device)  # normalized to gridspace gain
    gain = torch.ones(43, device=targets.device)
    ai = torch.arange(na, device=targets.device).float().view(na, 1).repeat(1, nt)  # same as .repeat_interleave(nt)
   # aaa=targets.repeat(na, 1, 1)
    ...
    for i in range(det.nl):
        ...
        #landmarks 10
        gain[6:30] = torch.tensor(p[i].shape)[[3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2]]  # xyxy gain
       ...
       # print("lks=",lks[0])
        ...
        lks[:, [6, 7]] = (lks[:, [6, 7]] - gij)
        lks[:, [8, 9]] = (lks[:, [8, 9]] - gij)
        lks[:, [10, 11]] = (lks[:, [10, 11]] - gij)
        lks[:, [12, 13]] = (lks[:, [12, 13]] - gij)
        lks[:, [14, 15]] = (lks[:, [14, 15]] - gij)
        lks[:, [16, 17]] = (lks[:, [16, 17]] - gij)
        lks[:, [18, 19]] = (lks[:, [18, 19]] - gij)
        lks[:, [20, 21]] = (lks[:, [20, 21]] - gij)
        lks[:, [22, 23]] = (lks[:, [22, 23]] - gij)

        conf= t[:,30:42]
       # print("point_conf=",conf[0])
        ...
        conf_point.append(conf)
        #print('lks: ',  lks.size())

    return tcls, tbox, indices, anch, landmarks, lmks_mask,conf_point
