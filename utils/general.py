...
def non_max_suppression_face(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, labels=()):
    ...
    nc = prediction.shape[2] - 29 -12 # number of classes
    ...
    t = time.time()
    output = [torch.zeros((0, 42), device=prediction.device)] * prediction.shape[0]
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if labels and len(labels[xi]):
            l = labels[xi]
            v = torch.zeros((len(l), nc + 29), device=x.device)
            v[:, :4] = l[:, 1:5]  # box
            v[:, 4] = 1.0  # conf
            v[range(len(l)), l[:, 0].long() + 29] = 1.0  # cls
            x = torch.cat((x, v), 0)
        ...
        # Compute conf
        x[:, 29:30] *= x[:, 4:5]  # conf = obj_conf * cls_conf
        ...
        if multi_label:
            i, j = (x[:, 29:41] > conf_thres).nonzero(as_tuple=False).T
            x = torch.cat((box[i], x[i, j + 29, None], x[:, 5:29] ,j[:, None].float()), 1)
        else:  # best class only
            conf, j = x[:, 29:41].max(1, keepdim=True)
            x = torch.cat((box, conf, x[:, 5:29], j.float()), 1)[conf.view(-1) > conf_thres]
        ...
        # Batched NMS
        c = x[:, 29:30] * (0 if agnostic else max_wh)  # classes
