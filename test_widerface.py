...
def detect(model, img0):
    ...
    if pred is not None:
        ...
        for j in range(pred.size()[0]):
            ...
            mask.append(landmarks)
    return boxes,mask


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', nargs='+', type=str, default='runs/train/exp15/weights/last.pt', help='model.pt path(s)')
    ...
    parser.add_argument('--save_folder', default='/home/py/code/github/yolov5-pose/EHF/re/', type=str, help='Dir to save txt results')
    parser.add_argument('--dataset_folder', default='/home/py/code/github/yolov5-pose/EHF/val/images/', type=str, help='dataset path')
    ...
    with torch.no_grad():
        ...
        for image_path in tqdm(glob.glob(os.path.join(testset_folder, '*'))):
            ...
            if img0 is None:
                ...
                continue
            boxes,mask = detect(model, img0)
            for i, box in enumerate(boxes):
                cv2.rectangle(img0, (box[0], box[1]), ((box[0]+box[2]), (box[1]+box[3])), (0, 255, 0), thickness=2, lineType=cv2.LINE_AA)
                for m in range(5):
                    point1 = int(mask[i][2 *m] *img0.shape[0]),int(mask[i][2 *m +1] *img0.shape[1])

                    cv2.circle(img0, point1,  3, (0,255,0), -1)
            cv2.imshow("eww",img0)
            cv2.waitKey(2000)
            # --------------------------------------------------------------------
